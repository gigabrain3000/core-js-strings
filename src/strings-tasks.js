...
function formatTime(minutes, seconds) {
  return seconds > 9
    ? ''.concat('0', minutes, ':', seconds)
    : ''.concat('0', minutes, ':0', seconds);
}
...
function invertCase(str) {
  return str
    .split('')
    .map((item) =>
      item.charCodeAt(0) > 90 ? item.toUpperCase() : item.toLowerCase()
    )
    .join('');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < result.length; i += 1) {
    if (
      (result[i].charCodeAt(0) > 64 && result[i].charCodeAt(0) <= 90) ||
      (result[i].charCodeAt(0) > 96 && result[i].charCodeAt(0) <= 122)
    ) {
      if (
        result[i].charCodeAt(0) > 109 ||
        (result[i].charCodeAt(0) > 77 && result[i].charCodeAt(0) <= 90)
      ) {
...
function getCardId(value) {
  ...
  return value.length > 2
    ? 9 + 13 * SUITS.findIndex((element) => element === value[2])
    : value
        .split('')
        .reduce(
          (acc, item) =>
            acc +
            (VALUES.findIndex((element) => element === item) + 0.5) +
            13 * (SUITS.findIndex((element) => element === item) + 0.5),
          0
        );
}
